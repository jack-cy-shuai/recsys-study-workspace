...
为验证方法的通用性，我们在矩阵分解（MF）上进行了同样的三组对比。如表~\ref{tab:mf} 所示，MF 上呈现出与 LightGCN 不同的有趣规律：简单的 LLM 相加注入反而使 MF 性能下降（0.0423 $\to$ 0.0396），而加入 HardBPR 后不仅恢复且大幅超越（0.0636，+50.5\%）。这表明：(1) HardBPR 的提升是\emph{模型无关}的，(2) 对于浅层模型，损失层的优化甚至比特征层的融合更为关键。

\begin{table}[ht]
    \centering
    \caption{MF 模型无关验证结果（Amazon-Book）}
    \label{tab:mf}
    \begin{tabular}{lrrr}
        \toprule
        方法 & Recall@20 & NDCG@20 & 相对 +LLM \\
        \midrule
        MF (random) & 0.0423 & 0.0260 & — \\
        MF+LLM & 0.0396 & 0.0249 & 基线 \\
        MF+LLM+HardBPR & \textbf{0.0636} & \textbf{0.0369} & \textbf{+60.6\%} \\
        \bottomrule
    \end{tabular}
\end{table}
...
消融实验揭示了一个反直觉的发现：语义硬负采样反而有害。我们通过 Information Bottleneck 框架给出了理论解释——数据层语义注入破坏了 BPR 训练的粗粒度压缩结构，而损失层语义加权在不改变压缩结构的前提下重新分配了训练资源。这一理论预测得到了实验验证（+Full 从 0.0664 退化为 0.0628）。

此外，我们在矩阵分解（MF）上的模型无关验证中观察到一个有趣的规律：简单的 LLM 相加注入对浅层 MF 反而有害（0.0423 $\to$ 0.0396），但 HardBPR 能完全翻转这一退化（0.0636，+60.6\%），进一步支持了"训练信号的优化比特征融合更重要"的核心论点。

未来工作将探索语义感知 BPR 在更多 backbone 上的泛化性，以及自适应 $\beta$ 调度策略以进一步提升训练效率。
