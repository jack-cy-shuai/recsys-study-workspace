...
\begin{figure}[ht]
    ...
    \fbox{\parbox{0.85\textwidth}{
        \vspace{1.5cm}
        \centering
        [训练流程示意图：ID Emb + Proj(LLM) → 图传播 → BPR 前向 → 语义相似度加权 → Loss]
        \vspace{0.5cm}
    }}
    \caption{语义感知 BPR 的训练流程。在标准 LightGCN-LLM 训练中，仅改动损失层的权重计算（红色框），其余部分（特征融合、图传播、负采样）均保持不变。}
    \label{fig:framework}
\end{figure}
...
\begin{figure}[ht]
    ...
    \fbox{\parbox{0.8\textwidth}{
        \vspace{2cm}
        \centering
        [分组柱状图：LGC / +LLM / +HardBPR-W 在冷/温/热三组的 Recall@20]
        \vspace{0.5cm}
    }}
    \caption{不同用户组上的 Recall@20 对比}
    \label{fig:cold_hot}
\end{figure}
