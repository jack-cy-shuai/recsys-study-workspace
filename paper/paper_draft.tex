...
\begin{abstract}
大语言模型（LLM）的语义嵌入已被广泛用于增强协同过滤推荐系统，但现有工作集中于在\emph{特征层面}将 LLM 语义注入模型，训练信号本身构成了一个独立且互补的瓶颈。本文指出，标准贝叶斯个性化排序（BPR）损失对所有正负样本对等权处理，导致大量训练迭代被耗费在区分语义不相干的\"容易\"负样本上（如区分科幻小说和菜谱）。与依赖模型内部打分或倾向性估计的已有 BPR 改进方法不同，我们提出语义感知 BPR 加权（HardBPR），利用冻结的 LLM 语义嵌入计算正负样本之间的语义相似度，并据此重新分配 BPR 损失的样本权重。该方法不增加模型参数、不需要额外推理成本，仅在损失函数层面做出改动。在 Amazon-Book、Steam 和 Yelp 三个公开数据集上，该方法在 LightGCN 上将 Recall@20 提升 44.7\%--101.1\%，在 MF 上提升 13.0\%，而对于 BPR 优化已饱和的模型（如 NGCF）则无明显增益——这界定了方法的适用范围。收益与数据稀疏性正相关，消融实验验证了各设计选择的有效性。
\end{abstract}
...
然而，上述工作的共同特点是：它们都对 LLM 语义嵌入做了精心设计，却全部继承了\emph{相同的训练范式}——标准的 BPR 损失配合随机负采样。这个训练范式存在一个独立于特征设计的瓶颈：随机负采样产生的绝大多数负样本都与正样本语义不相干，区分它们对模型几乎不产生有信息量的梯度，而真正需要细粒度区分的\"困难\"负样本（如《球状闪电》相对于《三体》）极少被采样到。
...
\textbf{语义硬负采样（数据层）} 则破坏了压缩结构。当 BPR 的三元组中负样本 $j$ 与正样本 $i$ 语义高度相似时，模型被迫在同类物品内部做细粒度判别。这等效于增大 IB 的 $\gamma$——引入了过多的 $I(X;T)$（低压缩），导致协同结构信号被语义噪声淹没。+Full 实验（HardBPR 叠加硬负采样）的退化（Recall@20 从 0.0397 降至 0.0289）直接验证了这一点。
...
未来工作将在更大规模数据集上验证语义感知 BPR——我们已处理完整 10-core Amazon-Book（16 万用户 $\times$ 13 万物品，自建 LLM embedding）并开源数据处理管线供社区基准测试——并研究自适应 $\beta$ 调度策略。
