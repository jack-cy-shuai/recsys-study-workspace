...
为验证方法的通用性，我们在矩阵分解（MF）和神经图协同过滤（NGCF）两种不同范式的 backbone 上进行了对比。如表~\ref{tab:mf} 所示，MF 上 HardBPR 将 Recall@20 从 0.0423 提升至 0.0478（+13.0\%）。NGCF 由于自身的特征变换已在 BPR 优化上趋于饱和，HardBPR 未带来额外提升（Amazon: 0.0881 $\to$ 0.0846；Steam: 0.1057 $\to$ 0.1078；Yelp: 0.0834 $\to$ 0.0835）——这表明 HardBPR 的价值在于弥补标准 BPR 的优化瓶颈，当模型自身已能充分优化 BPR 时，语义加权不再提供额外增益。
