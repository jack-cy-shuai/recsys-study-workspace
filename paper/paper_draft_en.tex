...
\begin{enumerate}
    ...
    \item We validate HardBPR across three datasets and three backbone architectures, demonstrating consistent improvements. We further validate on MovieLens-1M with \emph{self-generated} LLM embeddings and provide an Information Bottleneck analysis explaining \emph{why} loss-level semantic injection works while data-level injection does not.
\end{enumerate}
...
\end{equation}
where $\mathbf{s}_i, \mathbf{s}_j$ are the frozen LLM semantic embeddings of items $i$ and $j$, respectively. In our experiments, these embeddings come from RLMRec's pre-computed 1536-dimensional vectors (encoded via text-embedding-ada-002) for the main datasets, and from self-generated 384-dimensional vectors (encoded via all-MiniLM-L6-v2 with mean pooling) for the ML-1M cross-benchmark. The method is agnostic to the embedding source---any frozen semantic representation can be used.
...
\begin{table}[ht]
    ...
    \begin{tabular}{lrrrr}
        ...
        \midrule
        Amazon-Book\textsuperscript{*} & 11,000 & 9,332 & 120,464 & 0.12\% \\
        Steam\textsuperscript{*} & 23,310 & 5,237 & 316,190 & 0.26\% \\
        Yelp\textsuperscript{*} & 11,091 & 11,010 & 166,620 & 0.14\% \\
        ML-1M\textsuperscript{$\dagger$} & 6,040 & 3,706 & 988,129 & 4.41\% \\
        \bottomrule
    \end{tabular}\\[4pt]
    \footnotesize{\textsuperscript{*}Pre-computed LLM embeddings from RLMRec (1536-dim, text-embedding-ada-002). \textsuperscript{$\dagger$}Self-generated LLM embeddings (384-dim, all-MiniLM-L6-v2 with mean pooling).}
    \end{tabular}
\end{table}
...
\begin{table}[ht]
    \centering
    \caption{LightGCN results on three RLMRec datasets (pre-computed 1536-dim embeddings).}
    \label{tab:main}
...
We further compare against the gradient-reshaping Hard-BPR~\cite{Shi2024HardBPR} (denoted Shi-HardBPR), which modifies the sigmoid gradient curve using model-internal scores to suppress potential false negatives. Under our experimental setup, Shi-HardBPR achieves Recall@20 = 0.0267, identical to standard LightGCN---the bell-shaped gradient provides no benefit on this dataset, likely because the small-scale sparse interactions lack the severe false-negative patterns that motivate its design. This result underscores that external LLM semantic signals provide stronger and more reliable guidance than model-internal gradient reshaping alone.

We also explore two auxiliary enhancements: hot-to-cold collaborative distillation (+Distill) and semantic hard negative sampling (+Full). Distillation yields marginal gains (only 4.6\% of items are cold-start), while hard negative sampling actually causes \emph{performance degradation}. This directly validates our core thesis: \emph{semantic signals are a safe and effective focusing mechanism at the loss level, but can disrupt coarse-grained collaborative structure learning at the data level}.

\begin{table}[ht]
    \centering
    \caption{Ablation results on Amazon-Book (RLMRec pre-computed embeddings).}
    \label{tab:ablation}
    \begin{tabular}{lrr}
        \toprule
        Method & R@20 & N@20 \\
        ...
        Shi-HardBPR~\cite{Shi2024HardBPR} & 0.0267 & 0.0126 \\
...
\subsection{Cross-Benchmark Validation on ML-1M}

To verify that HardBPR generalizes beyond the RLMRec-provided embeddings, we conduct an additional experiment on MovieLens-1M (6,040 users $\times$ 3,706 items $\times$ 1M ratings, density $4.4\%$). Crucially, unlike the three RLMRec datasets where pre-computed embeddings are used, \textbf{we generate the LLM semantic embeddings ourselves} from scratch: we encode each movie's title and genre metadata (e.g., ``Toy Story (1995). Genres: Animation{\textbar}Children's{\textbar}Comedy'') using \texttt{sentence-transformers/all-MiniLM-L6-v2} (384-dimensional) with mean pooling over the final hidden states. User embeddings are computed as the mean of interacted item embeddings.

As shown in Table~\ref{tab:ml1m}, HardBPR achieves a small but consistent improvement over standard BPR (+1.2\% Recall@20). The gain is substantially smaller than on the RLMRec datasets because ML-1M is far denser (4.4\% vs.\ 0.12\%--0.26\%)---when collaborative signals are abundant, the BPR bottleneck is less severe, and semantic reweighting provides only marginal additional benefit. This experiment serves two purposes: (1) it demonstrates that HardBPR works with \emph{self-generated} embeddings rather than relying on a specific pre-computed source, and (2) it confirms that the gain is largest where BPR training is most starved for signal.

\begin{table}[ht]
    \centering
    \caption{Cross-benchmark validation on ML-1M (self-generated embeddings).}
    \label{tab:ml1m}
    \begin{tabular}{lrrrr}
        \toprule
        Method & R@10 & R@20 & N@20 & R@50 \\
        \midrule
        LightGCN (standard BPR) & 0.0361 & 0.0692 & 0.0260 & 0.1377 \\
        +HardBPR (ours) & \textbf{0.0368} & \textbf{0.0700} & \textbf{0.0262} & 0.1364 \\
        \bottomrule
    \end{tabular}
\end{table}
...
Future work will explore HardBPR on larger-scale datasets---we have processed the full 10-core Amazon-Book (160K users $\times$ 131K items) with self-generated embeddings and release the pipeline for community benchmarking---and investigate adaptive $\beta$ scheduling strategies.
