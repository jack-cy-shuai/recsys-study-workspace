...
\begin{abstract}
While large language model (LLM) embeddings have been widely adopted to enhance collaborative filtering recommenders, existing work focuses on injecting LLM semantics at the \emph{feature level}, overlooking a critical bottleneck in the training signal itself. We identify that standard Bayesian Personalized Ranking (BPR) loss treats all positive-negative sample pairs equally, causing roughly 80\% of training iterations to be wasted on distinguishing semantically unrelated ``easy'' negatives (e.g., distinguishing a sci-fi novel from a cookbook). We propose \emph{HardBPR}, a semantic-aware reweighting of the BPR loss that uses frozen LLM item embeddings to compute pairwise semantic similarity and reassigns sample weights accordingly---without adding parameters, without modifying model architecture, and without any LLM inference overhead. Experiments on three public datasets (Amazon-Book, Steam, Yelp) with three backbone architectures (LightGCN, MF, NGCF) demonstrate that HardBPR consistently improves Recall@20 by 13.0\%--101.1\% over standard BPR training. We further provide an Information Bottleneck analysis explaining why semantic signals are beneficial at the loss level yet harmful at the data level, establishing a design principle for future semantic enhancement methods.
\end{abstract}
...
\begin{figure}[ht]
    ...
    \caption{Gradient distribution comparison. Standard BPR (left): most samples have near-zero gradients. HardBPR (right): semantic reweighting produces a more uniform gradient distribution by amplifying hard samples.}
    \label{fig:grad}
\end{figure}
...
\paragraph{Datasets.} We use the three preprocessed datasets released by RLMRec~\cite{Ren2024RLMRec}. All datasets are chronologically split using the leave-one-out protocol, with LLM semantic embedding dimension of 1536. Statistics are shown in Table~\ref{tab:datasets}.
